...
get_cmd:				; Main processing loop
	...
	mov di, exit_string		; 'REBOOT' entered?
	...
	mov di, cat_string		; 'READ' entered?
	call os_string_compare
	jc near cat_file
	...
	mov di, new_string
	call os_string_compare		; 'NEW' entered?
	jc new_file
	...
	mov di, send_string		; 'SEND' entered?
	call os_string_compare
	jc send_data
...
.loop:						;loop through characters
	...
	call os_print_string
	jmp .end	
.end:						;send some characters that mean newline
	mov al, 10
	call os_send_via_serial
	mov al, 13
	call os_send_via_serial
...
; ------------------------------------------------------------------
new_file:
	mov word si, [param_list]	;check if a filename was provided
	call os_string_parse
	cmp ax, 0x00
...
.not_found:
	mov si, existant_file
	call os_print_string
	call os_draw_newline
	jmp get_cmd

creation_success:
	db "Success creating file ",0x00
existant_file:
	db "File already exists",0x00
...
; =====================================================================
	...
	version_msg		db 'TriadOS ', MIKEOS_VER, 13, 10, 0

	exit_string		db 'REBOOT', 0
	...
	send_string		db 'SEND', 0
	receive_string		db 'RECEIVE', 0

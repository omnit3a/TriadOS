...
no_autorun_bin:				;launch command line if there is no autorun.bin file
	call launch_command_line
...
define_palette:
	...
	mov dh, 0x00			; Red (0 to 63)
	mov ch, 0x0A			; Green ( 0 to 63)
	mov cl, 0x0C			; Blue (0 to 63)
	int 0x10
	
	mov ax, 0x1010			; set color palette
	mov bx, 0x07			; color to define (light grey)
	mov dh, 0x00			; Red (0 to 63)
	mov ch, 0x35			; Green ( 0 to 63)
	mov cl, 0x2A			; Blue (0 to 63)
	int 0x10
	
	mov ax, 0x1010			; set color palette
	mov bx, 0x04			; color to define (dark red)
	...
	mov ax, 0x1010			; set color palette
	mov bx, 0x0C			; color to define (light red)
...
not_bas_extension:
	...
	kerndlg_string_1	db 'Cannot load and execute TriadOS kernel!', 0
	kerndlg_string_2	db 'KERNEL.BIN is the core of TriadOS, and', 0
	...
	program_finished_msg	db 'Press a key to continue...', 0
